...
const FAIL_COPY: Record<PeekFail['reason'], { titleKey: string; bodyKey: string }> = {
  not_found: {
    titleKey: 'join.fail.notFound.title',
    bodyKey: 'join.fail.notFound.body',
  },
  used: {
    titleKey: 'join.fail.used.title',
    bodyKey: 'join.fail.used.body',
  },
  expired: {
    titleKey: 'join.fail.expired.title',
    bodyKey: 'join.fail.expired.body',
  },
  server_error: {
    titleKey: 'join.fail.serverError.title',
    bodyKey: 'join.fail.serverError.body',
  },
};
...
export default function JoinPage() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              t('join.conflictFallback'),
          );
        } else {
          toast.error(payload.error || t('join.toastAcceptFailed'));
        }
        setAccepting(false);
        return;
      }
      toast.success(t('join.toastWelcome'));
      ...
      toast.error(t('join.toastServerUnreachable'));
      setAccepting(false);
    }
  }, [token, t]);
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('join.toastSignOutFailed'));
      setSigningOut(false);
    }
  }, [t]);
    ...
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardContent className="flex flex-col items-center gap-3 py-12">
          ...
          <p className="text-sm text-muted-foreground">{t('join.verifying')}</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardHeader className="items-center text-center">
          ...
          <CardTitle className="text-xl text-foreground">{t(copy.titleKey)}</CardTitle>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('join.conflictTitlePrefix')} {peek.account_name}{' '}
                {t('join.conflictTitleSuffix')}
              </DialogTitle>
