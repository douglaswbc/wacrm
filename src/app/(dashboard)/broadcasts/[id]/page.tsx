...
function FunnelChart({ steps }: { steps: FunnelStep[] }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="mb-4 text-sm font-medium text-foreground">{t('broadcasts.funnel')}</h3>
...
export default function BroadcastDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('broadcasts.loadFailedDetail'));
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (delErr) {
      toast.error(`${t('broadcasts.toastDeleteFailed')}: ${delErr.message}`);
      return;
    }
    toast.success(t('broadcasts.toastDeleted'));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-sm">
            ...
            >
              {deleting ? t('broadcasts.deleting') : t('broadcasts.confirm')}
            </Button>
          </div>
        ) : (
          <Button
            ...
            title={
              broadcast.status === 'sending'
                ? t('broadcasts.cannotDelete')
                : t('broadcasts.deleteTooltip')
            }
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <StatCard
          label={t('broadcasts.totalRecipients')}
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
          <h2 className="text-sm font-medium text-foreground">
            {t('broadcasts.recipients')} ({filteredRecipients.length}
            {statusFilter !== 'all' ? ` / ${recipients.length}` : ''})
          </h2>
          ...
          <div className="flex h-32 items-center justify-center">
            <p className="text-sm text-muted-foreground">
              {recipients.length === 0
                ...
                : t('broadcasts.noRecipientsFilter')}
            </p>
          </div>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-muted-foreground">{t('broadcasts.tableContact')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('broadcasts.tablePhone')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('broadcasts.tableStatus')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('broadcasts.tableSent')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('broadcasts.tableDelivered')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('broadcasts.tableRead')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('broadcasts.tableError')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
