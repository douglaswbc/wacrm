...
export default function AutomationsPage() {
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? t('automations.toastUpdateFailed'))
      return
    }
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t('automations.toastDuplicateFailed'))
      return
    }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t('automations.toastDeleteFailed'))
      return
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {showTemplates && (
        <section>
          <h2 className="mb-3 text-sm font-semibold text-muted-foreground">{t('automations.templates')}</h2>
      ...
      {automations.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card/40">
          ...
          <p className="mt-3 text-sm font-medium text-foreground">{t('automations.empty')}</p>
      ...
      <Dialog open={!!pendingDelete} onOpenChange={(v) => !v && setPendingDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('automations.deleteConfirm').replace('{name}', pendingDelete?.name ?? '')}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {automation.is_active && (
              <span className="relative flex h-2 w-2" aria-label={t('automations.activeLabel')}>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>{t('automations.last').replace('{time}', formatRelative(automation.last_executed_at))}</span>
          </div>
        </button>
