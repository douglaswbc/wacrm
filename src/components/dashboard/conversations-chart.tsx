      ...
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-muted/60 p-1">
          {[7, 30, 90].map((r) => (
            ...
            >
              {t('dashboard.conversationsChart.days', r)}
            </button>
          ))}
        </div>
      </header>
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      <svg
        ...
        role="img"
        aria-label={t('dashboard.conversationsChart.ariaPerDay')}
      >
        ...
        >
          ...
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-blue-300">
              ...
              {t('dashboard.conversationsChart.incomingCount', hovered.incoming)}
            </span>
            <span className="flex items-center gap-1.5 text-primary">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-primary" />
              {t('dashboard.conversationsChart.outgoingCount', hovered.outgoing)}
            </span>
          </div>
        </div>
      )}
