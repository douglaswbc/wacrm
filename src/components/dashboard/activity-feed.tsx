  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground">{t('dashboard.activityFeed.title')}</h2>
        <Link
          href="/inbox"
      ...
      {loading || !items ? (
        ...
        <div className="p-5">
          <EmptyState
            icon={Inbox}
            title={t('dashboard.activityFeed.emptyTitle')}
            hint={t('dashboard.activityFeed.emptyHint')}
          />
        </div>
      ) : (
        <>
          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-3 text-xs">
            <span className="text-muted-foreground tabular-nums">
              {t('dashboard.activityFeed.showing', visible.length)} {totalLoaded}
              {totalLoaded === 50 ? '+' : ''}
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1 text-muted-foreground">{t('dashboard.activityFeed.show')}</span>
...
function relativeTime(iso: string, t: TranslateFn): string {
  ...
  if (diffSec < 60) return t('dashboard.activityFeed.secondsAgo', Math.max(1, diffSec))
  if (diffSec < 3600) return t('dashboard.activityFeed.minutesAgo', Math.floor(diffSec / 60))
  if (diffSec < 86400) return t('dashboard.activityFeed.hoursAgo', Math.floor(diffSec / 3600))
  if (diffSec < 2_592_000) return t('dashboard.activityFeed.daysAgo', Math.floor(diffSec / 86400))
