              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-muted-foreground tabular-nums">
                    {s.dealCount === 1
                      ? t('dashboard.pipelineDonut.dealOne', s.dealCount)
                      : t('dashboard.pipelineDonut.deals', s.dealCount)}
                  </span>
...
function Donut({ data, currency }: { data: PipelineDonutData; currency: string }) {
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label={t('dashboard.pipelineDonut.ariaValueByStage')}>
        {/* background ring */}
        ...
        >
          {t('dashboard.pipelineDonut.total')}
        </text>
        <text
          x={cx}
          y={cy + 14}
