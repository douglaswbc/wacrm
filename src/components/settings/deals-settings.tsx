...
export function DealsSettings() {
  ...
  async function handleSave() {
    ...
    const { error } = await supabase
      ...
      .eq("id", accountId);
    if (error) {
      toast.error(t("dealsSettings.toastSaveFailed"));
      setSaving(false);
      return;
    }
    ...
    setSaving(false);
    toast.success(t("dealsSettings.toastUpdated"));
  }
  ...
  return (
    <section className="max-w-2xl animate-in fade-in-50 duration-200">
      <SettingsPanelHead
        ...
        description={t("dealsSettings.description")}
      />
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-foreground">
            <Coins className="size-4 text-primary" />
            {t("dealsSettings.defaultCurrency")}
          </CardTitle>
          <CardDescription className="text-muted-foreground">
            {t("dealsSettings.cardDescription")}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-2 sm:max-w-xs">
            <Label className="text-muted-foreground">{t("dealsSettings.currency")}</Label>
            <select
              value={selected}
            ...
            >
              {saving ? (
                <>
                  ...
                  {t("dealsSettings.saving")}
                </>
              ) : (
                t("dealsSettings.save")
              )}
            </Button>
          )}
        </CardContent>
