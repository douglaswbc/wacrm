...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('apiKeys.loadFailed'));
        return;
      }
      ...
      toast.error(t('apiKeys.serverUnreachable'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('apiKeys.revokeFailed'));
        return;
      }
      toast.success(`${t('apiKeys.revoked')} "${key.name}"`);
          ...
          <CardContent className="flex flex-col items-center justify-center py-10 text-center">
            ...
            {canEditSettings ? (
              <p className="text-muted-foreground mt-1 text-xs">
                {t('apiKeys.emptyClickBefore')}{' '}
                <span className="text-foreground">{t('apiKeys.newKey')}</span>{' '}
                {t('apiKeys.emptyClickAfter')}.
              </p>
            ) : (
              <p className="text-muted-foreground mt-1 text-xs">
                {t('apiKeys.emptyAskAdmin')}
              </p>
            )}
          </CardContent>
        </Card>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground mt-1.5 text-xs">
                        {t('contacts.created')} {fmtDate(k.created_at)}
                        {' · '}
                        {k.last_used_at
                          ? `${t('apiKeys.lastUsed')} ${fmtDate(k.last_used_at)}`
                          : t('apiKeys.neverUsed')}
                        {k.expires_at && status !== 'expired'
                          ? ` · ${t('apiKeys.expires')} ${fmtDate(k.expires_at)}`
                          : ''}
                      </p>
                    </div>
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || t('apiKeys.createFailed'));
        return;
      }
            ...
            <div className="max-h-[55vh] overflow-y-auto -mx-1 px-1 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground text-xs">
                  {t('apiKeys.noScopesBefore')}{' '}
                  <code className="text-[11px]">GET /api/v1/me</code>{' '}
                  {t('apiKeys.noScopesAfter')}
                </p>
              </div>
            </div>
