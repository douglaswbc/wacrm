...
export function MessageComposer({
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === "ai_not_configured") {
          toast.error(t('inbox.aiNotConfigured'));
        } else {
          toast.error(data.error ?? t('inbox.draftFailed'));
        }
        return;
      }
      const draftText = typeof data.draft === "string" ? data.draft.trim() : "";
      if (!draftText) {
        toast.error(t('inbox.draftEmpty'));
        return;
      }
      setText(draftText);
      ...
      toast.error(t('inbox.aiUnreachable'));
    } finally {
      setDrafting(false);
    }
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(
          `${t('inbox.fileTooLargeStart')} ${(file.size / 1024 / 1024).toFixed(1)} MB — ${kind} ${t('inbox.fileTooLargeLimit')} ${Math.round(
            max / 1024 / 1024,
          )} MB.`,
        );
        return;
      ...
      try {
        ...
        toast.error(err instanceof Error ? err.message : t('inbox.uploadFailed'));
      } finally {
        setBusy(false);
      }
    },
  ...
  const finalizeRecording = useCallback(
    async (bytes: Uint8Array) => {
      ...
      if (file.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
        toast.error(t('inbox.recordingTooLong'));
        return;
      }
      setBusy(true);
      try {
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error(t('inbox.voiceNotSupported'));
      return;
    }
    try {
      ...
      {draft ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-muted px-4 py-2.5">
          ...
          <span className="flex-1 text-sm text-foreground">
            {t('inbox.recording')} {formatDuration(recordSeconds)} /{" "}
        ...
        <div className="flex items-end gap-2">
          ...
          <GatedButton
            ...
            size="sm"
            canAct={!readOnly}
            gateReason={t("inbox.sendMessages")}
          ...
          <GatedButton
            ...
            gateReason={t("inbox.sendMessages")}
            disabled={drafting}
            title={readOnly ? undefined : t('inbox.draftWithAiTitle')}
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ? t('inbox.readOnlyPlaceholder')
                : sessionExpired
                  ? t('inbox.expiredPlaceholder')
                  : t('inbox.placeholder')
            }
            ...
            title={readOnly ? t('inbox.readOnlyCantSend') : undefined}
            className={cn(
      ...
      <div className="mt-2 flex items-end gap-2">
        {draft.kind !== "audio" && (
          <input
            ...
            placeholder={t('inbox.captionPlaceholder')}
