...
export function ContactDetailView({
  ...
  async function saveDetails() {
    ...
    if (error) {
      toast.error(t('contacts.toastUpdateFailed'));
    } else {
  ...
  async function addNote() {
    ...
    if (error) {
      toast.error(t('contacts.noteAddFailed'));
    } else {
      setNewNote('');
      fetchNotes();
  ...
  async function deleteNote(noteId: string) {
    ...
    if (error) {
      toast.error(t('contacts.noteDeleteFailed'));
    } else {
  ...
  async function saveCustomFields() {
    ...
    try {
      ...
      toast.error(t('contacts.customFieldsSaveFailed'));
    }
  ...
  async function handleSendTemplate(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(`${t('contacts.templateSendFailed')}: ${reason}`);
        return;
      }

      toast.success(`${t('contacts.templateSentPre')} "${template.name}" ${t('contacts.templateSentPost')}`);
    } catch (err) {
      const reason = err instanceof Error ? err.message : t('contacts.networkError');
      toast.error(`${t('contacts.templateSendFailed')}: ${reason}`);
    } finally {
  ...
  async function handleMediaLibrarySelect(
    ...
    try {
      ...
      if (!res.ok) {
        const reason = payload?.error || `HTTP ${res.status}`;
        toast.error(`${t('contacts.mediaSendFailed')}: ${reason}`);
        return;
      }
      toast.success(t('contacts.mediaSent'));
    } catch (err) {
      const reason = err instanceof Error ? err.message : t('contacts.networkError');
      toast.error(`${t('contacts.mediaSendFailed')}: ${reason}`);
    } finally {
            ...
            <Tabs defaultValue="details" className="flex-1 flex flex-col min-h-0">
              ...
              <TabsContent value="details" className="flex-1 overflow-y-auto px-4 py-3">
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <Label className="text-muted-foreground text-xs">{t('contacts.name')}</Label>
                    <Input
                      value={editName}
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-muted-foreground text-xs">{t('contacts.email')}</Label>
                    <Input
                      value={editEmail}
                    ...
                    notes.map((note) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground mt-1.5">
                          {new Date(note.created_at).toLocaleDateString(toLocale(language), {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric',
                            hour: '2-digit',
                    ...
                    {customFields.map((field) => (
                      <div key={field.id} className="space-y-1.5">
                        ...
                        <Input
                          ...
                          placeholder={`${t('contacts.enterValuePrefix')} ${field.field_name}...`}
              ...
              <TabsContent value="deals" className="flex-1 overflow-y-auto px-4 py-3">
                {loadingDeals ? (
                  ...
                  <p className="text-xs text-muted-foreground">{t('contacts.noDeals')}</p>
                ) : (
