...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (!name) return;
    if (!accountId || !user) {
      toast.error(t('contacts.profileNotLinked'));
      return;
    }
    if (isDuplicate(name)) {
      toast.error(`${t('contacts.fieldDupPre')}${name}${t('contacts.fieldDupPost')}`);
      return;
    }
    ...
    if (error) {
      toast.error(t('contacts.fieldCreatePermFailed'));
      return;
    }
    toast.success(`${t('contacts.fieldCreatedPre')}${name}${t('contacts.fieldCreatedPost')}`);
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.fieldRenameFailed'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `${t('contacts.fieldDeleteConfirmPre')}${field.field_name}${t('contacts.fieldDeleteConfirmPost')}`
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.fieldDeleteFailed'));
      return;
    }
    toast.success(`${t('contacts.fieldDeletedPre')}${field.field_name}${t('contacts.fieldDeletedPost')}`);
    await fetchFields();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={t('contacts.fieldNamePlaceholder')}
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={`${t('contacts.renameAria')} ${field.field_name}`}
